...
class HumanDataCollector:
    def __init__(self, args):
        ...
        if self.args.collect_data:
            ...
            # self.exp_data_folder = self.human_data_folder + '/' + self.args.exp_name + '/human/' + current_time
            # if not os.path.exists(self.exp_data_folder):
            #     os.makedirs(self.exp_data_folder)
           
            self.human_data_folder = '/home/robot/drive/robotool'
            self.exp_name = self.args.exp_name
            current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
            self.exp_data_folder = self.human_data_folder + '/videos_0903/' + self.args.exp_name + '/head/' + current_time
            if not os.path.exists(self.exp_data_folder):
                os.makedirs(self.exp_data_folder)
    ...
    def collect_data(self, event=None):
        ...
        if self.on_reset:
            ...
            if left_pinch_dist0 < 0.008 and left_pinch_dist1 > 0.05 and left_pinch_dist2 > 0.05 and left_pinch_dist3 > 0.05:
                ...
                episode_num = self.record_episode_counter + 1
                ...
                rospy.loginfo(f"Started data collection for episode {episode_num}")
