...
class SynchronizedCameraRecorder:
    def __init__(self, camera_serial, name, desired_fps=25, use_background_saving=True, queue_size=700, 
        ...
        if use_fast_writer:
            rospy.loginfo(f"AVP Camera Recorder initialized with fast background saving (target FPS: {desired_fps}, queue size: {queue_size}, workers: {num_workers})")
        elif use_background_saving:
            rospy.loginfo(f"AVP Camera Recorder initialized with background saving (target FPS: {desired_fps}, queue size: {queue_size})")
        else:
            rospy.loginfo(f"AVP Camera Recorder initialized with direct saving (target FPS: {desired_fps})")
    ...
    def collection_status_callback(self, msg):
        ...
        try:
            ...
            if status == "READY":
                ...
                self.session_dir = os.path.join(f"/home/robot/drive/robotool/videos_0903/{self.name}", f"{self.current_time}_{self.name}_{episode_num}")
