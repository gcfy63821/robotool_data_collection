...
from src.robotool.utils.timestamp_accumulator import get_accumulate_timestamp_idxs
from src.robotool.shared_memory.shared_ndarray import SharedNDArray
from src.robotool.shared_memory.shared_memory_ring_buffer import SharedMemoryRingBuffer
from src.robotool.shared_memory.shared_memory_queue import SharedMemoryQueue, Full, Empty
...
class SingleRealsense(mp.Process):
    ...
    def __init__(
        ...
        if enable_infrared:
            examples['infrared_1'] = np.empty(
                shape=shape, dtype=np.uint8)
            examples['infrared_2'] = np.empty(
                shape=shape, dtype=np.uint8)
    ...
    def run(self):
        ...
        try:
            ...
            iter_idx = 0
            t_start = time.time()
            # print ir versus rgb intrinsics
            print(f"Serial number: {self.serial_number}")
            if self.enable_infrared:
                ir_prof_1 = pipeline_profile.get_stream(rs.stream.infrared, 1).as_video_stream_profile().get_intrinsics()
                print(f"IR Intrinsics: {ir_prof_1}")
            rgb_intrinsics = pipeline_profile.get_stream(rs.stream.color).as_video_stream_profile().get_intrinsics()
            ...
            while not self.stop_event.is_set():
                ...
                if self.enable_infrared:
                    ir_frame_1 = frameset.get_infrared_frame(1).get_data()
                    ir_frame_2 = frameset.get_infrared_frame(2).get_data()
                    data['infrared_1'] = np.asanyarray(ir_frame_1)
                    data['infrared_2'] = np.asanyarray(ir_frame_2)
